...
public class Main : Object {
	...
	public Main () {
		try {
            ...
            messagedialog = builder.get_object ("messagedialog") as MessageDialog;
    ...
    [CCode (instance_pos = -1)]
    public void on_messagedialog_close (MessageDialog source) {
        messagedialog.hide();
    }
        ...
        try {
            ...
            if (regex.match((string) message.response_body.data, 0, out match_info)) {
                while (match_info.matches()) {
                    ...
                    //stdout.printf(match_info.fetch(0));
                    match_info.next();
                }
            } else {
